...
subplotMeans = c(2, 4, 8, 8, 16, 32, 32, 64)
yearFX = runif(n= 10, 0.5, 1.5)
yearFX
...
for (i in 1:max(year)){ 
  ...
  newCover = yearFX[i]*subplotMeans   
  print(newCover)
  df$cover[ which(df$year == i) ] = rnorm(length(subplot), newCover, abs(newCover*0.2)) #### This could represent measurement error 
}
...
partialPGRLong = stack(partialPGRTable, select = c(PGRLM, PGRMER))
partialPGRLong$year = unique(partialPGRTable$year)
partialPGRLong$Complete = 'Incomplete'
partialPGRLong


allPGR_estimates = rbind(partialPGRLong, PGRLong)
...
cor.test(partialPGRTable$PGRMER, PGRTable$PGR1) ##### correlation depends on data points missing 

cor.test(partialPGRTable$PGRLM, PGRTable$PGR1)
